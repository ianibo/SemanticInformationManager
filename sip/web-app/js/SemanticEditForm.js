...
function buildFormPanel(layout_definition, 
  ...
  new_div.attr('id',layout_definition.element_id);
  new_div.attr('class','ui-tabs-panel ui-widget-content ui-corner-bottom ui-tabs-hide');
...
function createAssocComboControl(parent_element,
  ...
  new_li.append(new_sel);
  parent_element.append(new_li);
  
  new_sel.attr('id',new_control_id);
  new_sel.attr('data-resource-uri',root_object_uri);
  new_sel.attr('data-property',propdef.property_uri);
  new_sel.attr('data-property-idx',""+i);
  new_sel.attr('data-propdef-idx',""+p);
  new_sel.attr('value',ref);
  new_sel.attr('onchange',"assocComboChanged(this,"+p+","+propdef.mandatory+","+propdef.cardinality+");");
  
  // Having created the select control, populate it with data from the data/list service
  populateAssocCombo(target_repository_id,
                     propdef.refTypeURI,
                     new_sel,
                     propdef.displayProps);
...
function populateAssocCombo(repository, type_uri, target_combo, display_props) {
  ...
  console.log("populateAssocCombo("+repository+","+type_uri+",%o,...)",target_combo);
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      for ( i in result ) {
        console.log("Adding result "+result[i].uri);
...
function createAssocListControl(parent_element,
  ...
  new_tab.attr('id',table_control_id);

  var new_thead = $(document.createElement('thead'));
  var headers = $(document.createElement('tr'));
  headers.attr('id',table_control_id+"_h");
  headers.append("<th>ChildURI</th>");

  var tbody = $(document.createElement('tbody'));
  tbody.attr('id',table_control_id+"_b");

  var tfoot = $(document.createElement('tfoot'));
  tfoot.attr('id',table_control_id+"_f");

  new_thead.append(headers);
  new_tab.append(new_thead);
  new_tab.append(tbody);
  new_tab.append(tfoot);

  new_li.append("New assoc_list \""+propdef.control+"\" label: \""+propdef.label+"\" of type "+propdef.type);
  ...
  var add_buttonJQ = $(document.createElement('button'));
  add_buttonJQ.attr('type','button');
  add_buttonJQ.attr('id',table_control_id+"_button");
  add_buttonJQ.html("Add");

  var footer_tr = $(document.createElement('tr'));
  var footer_td = $(document.createElement('td'));
  footer_td.attr('colspan',propdef.cols.length);
  tfoot.append(footer_tr.append(footer_td.append(add_buttonJQ)));

  // We now add a footer row with a button to pop up an add dialog (Search for rows / create)
  // tfoot.append("<tr><td colspan=\""+propdef.cols.length+"\"><button type=\"button\" id=\""+table_control_id+"_button\">Add</button></td></tr>");
