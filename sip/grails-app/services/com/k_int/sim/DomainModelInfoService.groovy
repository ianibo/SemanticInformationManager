...
class DomainModelInfoService {
    ...
    def sync() {
      ...
      log.debug("Looking up admin user...")
      ...
      log.debug("admin user: ${admin_user}")

      log.debug( "Syncing context models with local GORM domain classes....")
      ...
      grailsApplication.getArtefacts("Domain").each { domainclass ->
        ...
        if ( ctx == null ) {
          ...
          log.debug( "Create new default dynamic search context uri:gorm:${domainclass.fullName}/defaultsearch")
          ...
          log.debug( "Create new default dynamic edit context uri:gorm:${domainclass.fullName}/defaultedit")
